...
if ($VERI_WORK_DIR) {
   $VERI_DIR  = "$VERI_WORK_DIR/$this_cycle";  ######
   system("mkdir $VERI_DIR");
   print ("verify dir = $VERI_DIR\n");
} else {
   $VERI_DIR = "$RUNDIR/verify/$this_cycle";
   system("mkdir $VERI_DIR");
}
...
sub lncp_aux_file(){
    ...
    for($ihr=$start_hour; $ihr<=$end_hour; $ihr+=$incre_hour) {
        ...
        $filex1=&tool_date12_to_outfilename("auxhist3_d${domain}_", $datex, "");
        $pathx1="$dir_run/$filex1";
        $filex2=&tool_date12_to_outfilename("auxhist3_d${domain}_", $datex, ".nc4.p");
        ...
        if (-e $pathx1) {
            ...
            $file_name2_unpack=&tool_date12_to_outfilename("auxhist3_d${domain}_", "$datex", ".nc4");
...
sub lncp_wrfout_file(){
    ...
    for($ihr=$start_hour; $ihr<=$end_hour; $ihr+=$incre_hour) {
        ...
        if($ihr < 0 ) {
            $dir_run="$RUNDIR/$cycle/WRF_F/";
            $filex1=&tool_date12_to_outfilename("wrfout_d${domain}_", $datex, "");
            $pathx1="$dir_run/$filex1";
            $dir_run2="$RUNDIR/$cycle/";
            $filex2=&tool_date12_to_outfilename("wrfout_d${domain}_", $datex, ".${MEMBER}_F");
            $pathx2="$dir_run2/$filex2";
            $dir_arc="$HOMEDIR/data/cycles/$GMID/archive/$MEMBER/wrfout_upr/final";
            $filexa=&tool_date12_to_outfilename("wrfout_d${domain}_", $datex, ".${MEMBER}_F");
            ...
            $pathx1="$dir_run/$filex1";
            $dir_run2="$RUNDIR/$cycle/";
            $filex2=&tool_date12_to_outfilename("wrfout_d${domain}_", $datex, ".${MEMBER}_P+FCST");
            $pathx2="$dir_run2/$filex2";
            $dir_arc="$HOMEDIR/data/cycles/$GMID/archive/$MEMBER/wrfout_upr/$cycles";
            $filexa=&tool_date12_to_outfilename("wrfout_d${domain}_", $datex, ".${MEMBER}_P+FCST");
            $pathxa="$dir_arc/$filexa";
        }
        if (-e $pathx1) {
            print("$filex1: file exists in WRF_F/WRF_P run dir, to link\n");
